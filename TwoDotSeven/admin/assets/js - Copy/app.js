...
+function($) {
    ...
    Bjax.prototype.updateBar = function(per) {
        var that = this;
        this.$bar.stop().animate({width: per + '%'}, 500, 'linear', function() {
            if (per == 100)
                that.update();
        });
    }
    ...
    $.fn.bjax = function(option) {
        return this.each(function() {
            ...
            if (!data)
                $this.data('app.bjax', (data = new Bjax(this, options)))
            if (typeof option == 'string')
                data[option]()
        })
    }
    ...
    $(document).on('click.app.bjax.data-api', '[data-bjax], .nav-primary', function(e) {
        if (!Bjax.prototype.enable(e))
            return;
        var $this = $(this);
