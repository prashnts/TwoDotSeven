...
function GetUserListMarkup(Page) {
    var RESTURI = '/dev/direction/_markup_getUserList/'+(typeof Page === "undefined" ? 1: Page);
    $.ajax({
        type: 'GET',
        url: RESTURI,
        timeout: 30000,
        beforeSend: function() {
            $("#REST_UserManagementPageLocation").html('Page - of -');
            $("a[name=Pagination]").unbind('click');
            $('#REST_UserManagementTable').addClass('Blusr');
            $('#UserListAJAXLoader').fadeIn(200);
            $('#UserListAJAXLoadError').fadeOut(200);
            $('*').css('cursor','wait');
        },
        ...
        error: function() {
            $('#UserListAJAXLoader').fadeOut(200);
            $('#UserListAJAXLoadError').fadeIn(200);
            $('*').css('cursor','');
            $("a[name=Pagination]").click(function() {
                console.log($(this).attr('data-next'));
                GetUserListMarkup($(this).attr('data-next'));
...
PswdOA = false;
function ExecutePswdOA(ID) {
    if (PswdOA) {
        return;
    }
    POSTData = {
        UserName: $("#"+ID).attr('data-username'),
        NewPassword: $("#"+ID+'NewPswd').val(),
        ConfNewPassword: $("#"+ID+'ConfPswd').val()
    }
    $.ajax({
        type: 'POST',
        url: '/dev/direction/userPasswordOverride',
        timeout: 30000,
        data: POSTData,
        beforeSend: function() {
            PswdOA = true;
        ...
        success: function() {
            $("#"+ID).html('Executed Successfully');
            $("#"+ID).removeClass('btn-info');
            $("#"+ID).addClass('btn-success');
            setTimeout(function() {
                $("#"+ID).html('Execute Action');
        ...
        error: function(data) {
            $("#"+ID).html('Error Executing Action');
            $("#"+ID).removeClass('btn-info');
            $("#"+ID).addClass('btn-danger');
            setTimeout(function() {
                $("#"+ID).html('Please Retry or Check input');
...
EMailOA = false;
function ExecuteEMailOA(ID) {
    if (EMailOA) {
        return;
    }
    POSTData = {
        UserName: $("#"+ID).attr('data-username'),
        NewEMail: $("#"+ID+'NewEMail').val()
    }
    $.ajax({
        type: 'POST',
        url: '/dev/direction/userEMailOverride',
        timeout: 30000,
        data: POSTData,
        beforeSend: function() {
            EMailOA = true;
...
VerifyEmailToggle = false;
function VerifyEMail(ID) {

    if (VerifyEmailToggle) {
        console.log('here');
        return false;
    }
    ...
    POSTData = {
        UserName: $("#"+ID).attr('data-username'),
        Do: $("#"+ID).parent().children().eq(0).attr('checked') ? 'UNVERIFY' : 'VERIFY'
    }
    $.ajax({
        type: 'POST',
        url: '/dev/direction/userEmailValidate',
        timeout: 30000,
        data: POSTData,
        beforeSend: function() {
            VerifyEmailToggle = true;
            $('*').css('cursor','wait');
            $('#'+ID).css('background-color','#3598DB');
        },  
        success: function() {
            $('#'+ID).css('background-color','#30CD72');
            $('*').css('cursor','');
        ...
        error: function(data) {
            $('#'+ID).css('background-color','#E94C3D');
            setTimeout(function() {
                $('*').css('cursor','');
                VerifyEmailToggle = false;
                $('#'+ID).css('background-color','');
                $('#'+ID).parent().children().eq(0).removeAttr('checked');
            }, 1000);
        }
    });
}
...
function VerifyProfile(ID) {
    ...
    $.ajax({
        type: 'POST',
        url: '/dev/direction/userProfileValidate',
        timeout: 30000,
        data: POSTData,
        beforeSend: function() {
 ...
 * Forked for TwoDot7 by Prashant Sinha
 * Implements Dynamic Server Side Push-Pull of Tags.
 */
(function($) {
    ...
    privateMethods = {
        pushAjax : function (tagValue, opts) {
            if (opts.enableAjaxPush) {
                var POSTData = opts.pushAjaxParameter;
                POSTData['Tag'] = tagValue;
                $.ajax({
                    type: 'POST',
                    url: opts.pushAjax,
                    timeout: 30000,
        ...
        pullAjax : function (tagValue, opts) {
            if (opts.enableAjaxPull) {
                var POSTData = opts.pullAjaxParameter;
                POSTData['Tag'] = tagValue;
                $.ajax({
                    type: 'POST',
                    url: opts.pullAjax,
        ...
        applyDelimiter : function (e) {
            ...
            publicMethods.pushTag.call($self, tagValue);
            privateMethods.pushAjax(tagValue, opts);
            e.preventDefault();
        },
        ...
        spliceTag : function (tagId) {
            ...
            privateMethods.showOrHide.call($self);
            privateMethods.pullAjax(tagBeingRemoved, $self.data('opts'));
            //if (tagManagerOptions.maxTags > 0 && tlis.length < tagManagerOptions.maxTags) {
            //  obj.show();
            //}
        },
