<?php
namespace TwoDot7\Admin\AssetServer\Config;
#  _____                      _____                         
# /__   \__      _____       |___  |    ___               __    ____                    
#   / /\/\ \ /\ / / _ \         / /    / _ | ___ ___ ___ / /_  / __/__ _____  _____ ____
#  / /    \ V  V / (_) |  _    / /    / __ |(_-<(_-</ -_) __/ _\ \/ -_) __/ |/ / -_) __/
#  \/      \_/\_/ \___/  (_)  /_/    /_/ |_/___/___/\__/\__/ /___/\__/_/  |___/\__/_/   

const Logo = "/data/core/static/images/logo.png";

const BackgroundImageRoot = "/data/core/static/images/backgrounds/";
const BackgroundImagePrimary = "city-pri.jpg";
const BackgroundImageSecondary = "city-sec.jpg";
const TintRoot = "/data/core/static/images/tints/";
const TintRed = "TransparencyHackRed15p.png";
const TintGreen = "TransparencyHackGreen15p.png";
const TintBlue = "TransparencyHackBlue15p.png";